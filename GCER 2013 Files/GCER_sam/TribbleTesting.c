...
int main()
{
	...
	while(1){
		...
		if (cam_area(COL_GREEN) > 50){
			...
			x=mcenter.x;y=mcenter.y;//add some crazy stupid operations here or something, but w/e
			//find the angles
			...
			if (x != 0){
				...
				printf("r %f a %f\n", radius, angle);
				/*if (x > 0) right(angle,radius);
				else if (x < 0) left(angle,radius);*/
				
				/*if (x > 0) rightish(radius);
				else if (x < 0) leftish(radius);*/
			}
		}
		//beep();msleep(500);beep();
	}
	printf("Hello, World!\n");
	return 0;
}
